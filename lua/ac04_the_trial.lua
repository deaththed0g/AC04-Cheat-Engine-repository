...
- Convert code to ASM.
- Code a function that deletes duplicate main headers.
- Other mission data stuff.
...
local function pcsx2_version_check()
    ...
    for processID, processName in pairs(getProcessList()) do
        ...
        if processName == "pcsx2.exe" or processName == "pcsx2-qt.exe" then
            ...
            table.insert(process_found, processName)
            table.insert(process_found, processID)
...
if (Ac04Trial_pcsx2_id_ram_start[3] == nil) then
    ...
    if #Ac04Trial_gameCheck ~= 0 then
		...
		-- Check if the player is currently in a mission. If not, just create the "Jump to sequence" entry.
	
		if not (readBytes(Ac04Trial_pcsx2_id_ram_start[2] + 0x51E768) <= 5 and readBytes(Ac04Trial_pcsx2_id_ram_start[2] + 0x51E768) >= 0) then
...
if Ac04Trial_table_active then
    ...
    local ac04_misc_memory_addresses = create_header("Miscellaneous memory addresses", Ac04Trial_main_header, true)
	
	-- Check if the player is currently in a mission. If not, create the "Jump to sequence" entry only, else create the rest of the entries.
    ...
    if (readBytes(Ac04Trial_pcsx2_id_ram_start[2] + 0x51E768) <= 5 and readBytes(Ac04Trial_pcsx2_id_ram_start[2] + 0x51E768) >= 0) then
		...
		-- todo: Shrink the table to just the most common ID strings used in this demo.
		...
		for i = 1, 45 do
			...
			if iff_tag_value < 1 or iff_tag_value > 146 then
				...
				-- The "iff_tag_value" of the entity controlled by the player is always 0.
				-- Because there's no ID associated to this value I had to create a custom one ("PLAYER").
				-- Same goes for "UNKNOWN", which will be used if the "iff_tag_value" is higher than 145 (which is the total amount of ID strings used by the game).
				-- The reason for this is that some entities do not have any string ID associated to them.
...
if Ac04Trial_table_active then
    ...
    Ac04Trial_main_header = nil
    
    Ac04Trial_pcsx2_id_ram_start[2] = nil
